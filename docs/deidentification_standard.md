...
|-------------|-------------|--------------------------|
| **HIPAA Safe Harbor** (45 CFR §164.514(b)(2)) | Remove/generalize the 18 identifiers **and** have no actual knowledge of residual identifiability | `safe_harbor_strict` profile in [config/deid_rules.yaml](../config/deid_rules.yaml) — the only shipped profile that may make this claim. Zero derived values, so `assess_method_eligibility()` returns `safe_harbor`. Covers **both** sources: the MRN becomes an *assigned* `DEID-` surrogate under §164.514(c), not an MRN-derived token, so the crosswalk (in PHI-Vault) is the price of the linkage. Mapping in [docs/safe_harbor_mapping.md](safe_harbor_mapping.md). |
| **Safe Harbor prong (ii)** (§164.514(b)(2)(ii)) | No *actual knowledge* the residual data could identify an individual | Not automatable — no scan can establish it. The `actual_knowledge:` block in the rulebook ships **unsigned**; `load_actual_knowledge_attestation()` rejects placeholders, short statements and lapsed dates, and `NB_scorecard` **fails** a Safe Harbor claim until a named individual signs and dates it. Not evaluated for Expert Determination, whose §164.514(b)(1) risk analysis subsumes it. |
| ⚠️ `safe_harbor` profile (**misnamed**) | Safe Harbor *column treatments*, Expert Determination *claim* | Named for what it does to columns (dates → year, ZIP3, age cap), not for the claim you may make — it tokenizes MRNs so the two stars stay joinable. `assess_method_eligibility()` computes the claimable method from the rules and `NB_scorecard` hard-gates it. Use `safe_harbor_strict` if you need the claim. |
